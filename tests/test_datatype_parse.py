...
parse_row_testcases = {
    ...
    'row(min timestamp(6) with time zone, max timestamp(6) with time zone, null_count bigint)': ROW(dict(min=TIMESTAMP(timezone=True), max=TIMESTAMP(timezone=True), null_count=BIGINT())),
}
